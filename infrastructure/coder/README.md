...
- Workspace applications: private wildcard subdomains under
  `*.apps.coder.homelab.internal`, with tenant-compatible workspace domains
  routed through `coder-workspace-apps`
...
Workspace application buttons open private URLs such as:

```text
http://app.frontend-dev.tiborsuty.apps.coder.homelab.internal
```

The first label is available to applications as their tenant selector. For
example, after selecting the `metada` tenant, the same workspace is reached at
`http://metada.frontend-dev.tiborsuty.apps.coder.homelab.internal`. The app
gateway derives the stable Kubernetes Service name from the workspace and owner
labels. The browser must be connected to NetBird so the private wildcard DNS
record and route are available. Direct access is restricted by NetBird policy,
and the application still performs its own login. No local port-forward is
required.

Native Coder application hostnames under the less-specific
`*.apps.coder.homelab.internal` remain available for applications that do not
need tenant subdomains.
...
coder create frontend-dev \
  ...
  --parameter service_port=4300 \
  --parameter application_name=Frontend \
...
`../netbird/cloud/dns.tf` when creating another frontend workspace. When
`application_start_command` is non-empty, Coder starts it in the background on
every workspace start and stores its output in
`/tmp/coder-application.log`.
...
bind to `0.0.0.0` to accept connections from the gateway or another workspace.
The Service listens on port `80` and forwards to the workspace's configured
`service_port`.
