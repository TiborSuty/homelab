...
Workspace application buttons open private, owner-authenticated URLs such as:

```text
http://application--frontend-dev--tiborsuty.apps.coder.homelab.internal
```

The exact hostname is generated by Coder from the application, workspace, and
owner names. The browser must be connected to NetBird so the private wildcard
DNS record and route are available. No local port-forward is required.
...
Each workspace application is available from its authenticated button on the
workspace dashboard at its own private wildcard hostname. When
